  ...
  conn.ev.on('messages.upsert', async (mek) => {
    ...
    if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true") {
      ...
      try {
        ...
        if (error.errorcode === 'EKEYTYPE') {
          console.log('Skipping reaction due to cache error');
        }
      }
    }                       
    ...
    if (isCreator && mek.text.startsWith('%')) {
      ...
      try {
        ...
        let errorMsg = util.format(err);
        if (err.errorcode === 'EKEYTYPE') {
          reply(`Cache error: ${err.message}`);
        } else {
          reply(errorMsg);
        }
      }
    ...
    //================ownerreact==============
    if (senderNumber.includes("255789661031")) {
      if (isReact) return;
      try {
        m.react("👨‍💻");
      } catch (error) {
    ...
    if (isCmd) {
      ...
      if (cmd) {
        ...
        try {
          ...
          if (e.errorcode === 'EKEYTYPE') {
            console.log('Plugin cache error, skipping...');
          } else {
    ...
    events.commands.map(async (command) => {
      if (body && command.on === "body") {
        try {
          ...
          console.error('Error in on body command:', error.message);
        }
      } else if (mek.q && command.on === "text") {
        try {
          command.function(conn, mek, m, { from, l, quoted, body, isCmd, command, args, q, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply });
        } catch (error) {
          console.error('Error in on text command:', error.message);
        }
      } else if (
